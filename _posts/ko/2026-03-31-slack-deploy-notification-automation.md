...
|------|------|
| 트리거 | main 브랜치에 push |
| 빌드 | 서버: Gradle + Docker, 프론트: npm + Cloudflare Pages |
| 알림 | Slack Incoming Webhook (시작/완료/실패) |
| Release Note | `[release-note]` 태그 추출 -> 없으면 커밋 제목 fallback |
| Health Check | curl 루프 (60회 x 5초) |
| JSON 이스케이프 | Python `json.dumps` |
| 컨벤션 | 서브모듈당 1개 summary 커밋, 5줄 이내 |
