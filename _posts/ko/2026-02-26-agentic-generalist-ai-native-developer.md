...
물론 이 이름이 정착될지는 모른다. 하지만 적어도 **시장이 이 역할을 인식하기 시작했다**는 건 분명하다. 무신사가 코테에서 CLAUDE.md를 요구하고, Across가 "시스템화"를 핵심 역량으로 내세우고, 토스가 TPO라는 기술과 비즈니스를 동시에 다루는 역할을 정의한 것 — 이런 공고들이 하나의 방향을 가리키고 있다.
...
무신사 코테가 `CLAUDE.md`, `REQUIREMENTS.md`, `prompts/`를 필수로 요구한 건 아마 같은 결론에 도달했기 때문일 것이다 — AI 시대의 개발 역량은 코드가 아니라 **설계와 문서화에서 드러난다.**

## 이 직군이 확립될까

아직은 모른다. "Agentic Generalist"는 하나의 스타트업이 만든 이름이고, "AI Native Engineer"는 하나의 채용 프로그램에서 나온 이름이다.

하지만 이 두 공고가 가리키는 방향은 같다: **"코드를 잘 짜는 사람"이 아니라 "AI와 함께 시스템을 만드는 사람"**. AI가 코딩을 점점 더 잘하게 되면, 사람의 가치는 "무엇을 만들지 결정하고, AI가 잘 만들도록 시스템을 설계하는 것"으로 이동할 수밖에 없다.

솔직히, 이런 공고가 나오기 시작한 게 기쁘다. 주변에서 "이 분야의 선두주자"라고 말해줄 때마다 고마우면서도, 정작 그걸 증명할 수 있는 건 내 말뿐이었다. 그런데 이제 채용 시장이 이 역할에 이름을 붙이고, 코딩테스트에서 CLAUDE.md를 요구하기 시작했다. 내가 가고 있던 방향이 틀리지 않았다는 증거가 하나씩 나오는 느낌이다.
...
이 직군이 확립되면 좋겠다. 최소한 다음에 누가 "뭐 하시는 분이세요?"라고 물었을 때, "그게... PO인데 개발도 하고 납땜도 하고..."보다는 나은 답을 할 수 있을 테니.
...
2. **"바이브 코딩"은 시작이지 끝이 아니다.** 프로덕션에는 AI를 인프라로 쓰는 구조화된 프로세스가 필요하다.
3. **문서화도 시스템화할 수 있다.** 커스텀 커맨드로 프로세스를 코딩하면, 코드 변경 시 문서 동기화를 빠뜨릴 수 없다.
4. **"Agentic Generalist"라는 이름이 나왔다.** 직군으로 확립될지는 모르지만, 시장이 이 역할을 인식하기 시작한 건 분명하다.
