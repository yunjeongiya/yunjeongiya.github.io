...
토큰 50배 절약, Git 히스토리 보존, 블로그 작성 자동화.
...
2. **Claude 대화 스레드 관리 실패**
   - 병렬로 3개 기능 작업 → 대화 스레드 3개 생성
   - 며칠 지나면 제목 까먹음, 오래된 스레드는 목록에서 사라짐
   - 새 대화 시작하면 "뭘 하고 있었는지" 다시 설명해야 함
...
## 구현
Access Token과 Refresh Token을 분리해서...
```
...
1. **인증 플로우 다이어그램**
   - 파일명: `01-auth-flow.png`
   - 내용: 로그인부터 토큰 갱신까지의 흐름
   - 캡처 방법: draw.io나 Mermaid로 다이어그램 작성

📁 저장 위치: `docs/features/F001-user-auth/images/`
