...
개발 완료 후 2-3주가 지나면 맥락을 잃어서 블로그를 못 쓴다. `/blog` 명령어는 Git + 작업일지 + Features를 분석해 초안을 자동 생성하고, 스크린샷 추천을 HTML 주석으로 삽입한다. **블로그 작성 시간 83% 감소** (4시간 → 1시간), 작성 빈도 4배 증가.
...
이 문제를 해결하기 위해 **작업 완료 직후 블로그 초안을 자동으로 생성하는 `/blog` 명령어**를 만들었다.
...
오늘 날짜 + slugified 제목으로 파일명 자동 생성.

### 2. ezablocki.com - Cursor Slash Commands

재사용 가능한 AI 프롬프트를 프로젝트에 저장, Git으로 버전 관리, 팀 전체 공유.
...
같은 대화에서 `/blog` 실행 시 Git log를 다시 파싱할 필요가 없다.
...
새 대화에서 실행하면 Git log에서 `F0XX` 패턴 추출.

### 1.5단계: 기존 글 리서치 및 경쟁 분석

블로그 글 쓰기 전에 한국어/영어 기존 글을 서칭:
- 중복 글 방지
...
**핵심 혁신**: 스크린샷 추천을 별도 파일이 아닌 **blog.md 내부에 HTML 주석으로 삽입**
...
**해결**: 작업 완료 직후 자동으로 초안 생성.

### 2. 스크린샷은 "지금 아니면 안 된다"

UI는 계속 바뀐다. 2주만 지나도 동일한 화면을 재현할 수 없다.

**해결**: 스크린샷 추천을 초안에 주석으로 삽입 → 즉시 캡처 유도.
...
### 5. Slash 명령어는 "워크플로우 표준화" 도구

`/blog`를 Git에 커밋하면:
- 팀 전체가 동일한 방식으로 블로그 작성
- 명령어 개선이 모두에게 적용
- 신입 온보딩 간소화
